...
# create a stress test for the LSU

# get the issue width
if($ARGV[0] !~ /(\d+)/)
{
    print "You haven't specified an issue width!\n";
    exit(0);
}
else
{
...
open FILE, "> stress_lsu.s" or die "Could not open file\n";
...
for($i=0;$i<=32;$i++)
{
    ...
    print FILE "\tc0    mov \$r0.3 = $first\n\;\;\n";

    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    stw ((test_area + ", $j * 4, ") + 0)";
	#print FILE "[\$r0.0] = $first\n";
	print FILE "[\$r0.0] = \$r0.3\n";
    }
    print FILE "\;\;\n";
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    ldw \$r0.", $j+17, " = ((test_area + ", $j * 4, ") +0)";
	print FILE "[\$r0.0]\n";
    }
    print FILE "\;\;\n";

#	printf(FILE "\tc0    $op \$r0.9 = \$r0.3, 0x%x\n\;\;\n", $first_i);
#	print FILE "\tc0    $op \$r0.9 = \$r0.3, $first\n\;\;\n";
	$first |= 1 << $i;
}
...
for($i=0;$i<=16;$i++)
{
    ...
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    sth ((test_area + ", $j * 2, ") + 0)";
	#print FILE "[\$r0.0] = $first\n";
	print FILE "[\$r0.0] = \$r0.3\n";
    }
    print FILE "\;\;\n";
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    ldh \$r0.", $j+17, " = ((test_area + ", $j * 2, ") +0)";
	print FILE "[\$r0.0]\n";
    }
    print FILE "\;\;\n";
    ...
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    ldhu \$r0.", $j+17, " = ((test_area + ", $j * 2, ") +0)";
	print FILE "[\$r0.0]\n";
    }
    print FILE "\;\;\n";
...
for($i=0;$i<8;$i++)
{
    ...
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    stb ((test_area + $j) + 0)";
	#print FILE "[\$r0.0] = $first\n";
	print FILE "[\$r0.0] = \$r0.3\n";
    }
    print FILE "\;\;\n";
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    ldb \$r0.", $j+17, " = ((test_area + $j) +0)";
	print FILE "[\$r0.0]\n";
    }
    print FILE "\;\;\n";
    ...
    for($j=0;$j<$iw;$j++)
    {
	print FILE "\tc0    ldbu \$r0.", $j+17, " = ((test_area + $j) +0)";
	print FILE "[\$r0.0]\n";
    }
    print FILE "\;\;\n";
...
#system("/home/elds2/vex-3.41/lib/s2cs stress_lsu.s stress_lsu.cs.c -show_bundle_counts -trace_icache -trace_memory -runtime_icache_cfg -little_endian");
