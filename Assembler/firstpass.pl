...
sub first_pass()
{
    ...
    @file = <INPUT>;
    close INPUT;
    @pthread_file = split(/\//, $_[0]);
    $pthread_file[$#pthread_file] =~ s/(.+)\.s/pthread_$1.c.txt/;
    for($p_file = 0;$p_file<=$#pthread_file;$p_file++)
    {
	$pthread_file .= $pthread_file[$p_file];
	if($p_file < $#pthread_file)
	{
	    $pthread_file .= "/";
	}
    }
    open PTHREAD, "< $pthread_file" or die
	"Error: First Pass Failed\nCould not open file ($pthread_file): $!\n";
    while( <PTHREAD> )
    {
	chomp();
	($line_num, $func) = split(/\s+:\s+/);
	if(defined($line_num))
	{
	    $pthread{$line_num} = $func;
	}
    }
    close PTHREAD;
    $data_start = 0;
...
#			$temp_link =~ s/(\(*)_\?/$1$fil[$#fil]\_\?/g;
	...
	elsif($file[$i] =~ /^\s*(c(\d+)|\;\;)/)
	{
	    ...
	    {
		$temp_pthread = $1;
		if($pthread{$temp_pthread} eq "PTHREAD_CREATE")
		{
		    if($data =~ /call/)
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "PTHREAD_JOIN")
		{
		    if($data =~ /call/)
		    {
			$data .= " # $pthread{$temp_pthread}";
		...
		elsif($pthread{$temp_pthread} eq "PRINTF")
		{
		    if(($data =~ /call/) && ($data =~ /printf/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "CLOCK")
		{
		    if(($data =~ /call/) && ($data =~ /clock/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FOPEN")
		{
		    if(($data =~ /call/) && ($data =~ /fopen/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FSCANF")
		{
		    if(($data =~ /call/) && ($data =~ /fscanf/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FCLOSE")
		{
		    if(($data =~ /call/) && ($data =~ /fclose/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FEOF")
		{
		    if(($data =~ /call/) && ($data =~ /feof/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FFLUSH")
		{
		    if(($data =~ /call/) && ($data =~ /fflush/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FPRINTF")
		{
		    if(($data =~ /call/) && ($data =~ /fprintf/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FPUTC")
		{
		    if(($data =~ /call/) && ($data =~ /fputc/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FPUTS")
		{
		    if(($data =~ /call/) && ($data =~ /fputs/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FGETC")
		{
		    if(($data =~ /call/) && ($data =~ /fgetc/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FGETS")
		{
		    if(($data =~ /call/) && ($data =~ /fgets/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FREAD")
		{
		    if(($data =~ /call/) && ($data =~ /fread/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "FWRITE")
		{
		    if(($data =~ /call/) && ($data =~ /fwrite/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "SSCANF")
		{
		    if(($data =~ /call/) && ($data =~ /sscanf/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
		elsif($pthread{$temp_pthread} eq "SPRINTF")
		{
		    if(($data =~ /call/) && ($data =~ /sprintf/))
		    {
			$data .= " # $pthread{$temp_pthread}";
		    }
		}
