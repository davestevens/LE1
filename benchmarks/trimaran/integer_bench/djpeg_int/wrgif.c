...
{
  fputc(w & 0xFF, dinfo->pub.output_file);
  fputc((w >> 8) & 0xFF, dinfo->pub.output_file);
}
...
{
  fputc(val, dinfo->pub.output_file);
  fputc(val, dinfo->pub.output_file);
  fputc(val, dinfo->pub.output_file);
}
...
/* If colormap==NULL, synthesize a gray-scale colormap */
{
   ...
   */
  fputc('G', dinfo->pub.output_file);
  fputc('I', dinfo->pub.output_file);
  fputc('F', dinfo->pub.output_file);
  fputc('8', dinfo->pub.output_file);
  fputc('7', dinfo->pub.output_file);
  fputc('a', dinfo->pub.output_file);
  ...
  fputc(FlagByte, dinfo->pub.output_file);
  fputc(0, dinfo->pub.output_file); /* Background color index */
  fputc(0, dinfo->pub.output_file); /* Reserved (aspect ratio in GIF89) */
  ...
  for (i=0; i < ColorMapSize; i++) {
	...
	if (dinfo->cinfo->out_color_space == JCS_RGB) {
	  ...
	  fputc(GETJSAMPLE(colormap[0][i]) >> cshift, dinfo->pub.output_file);
	  fputc(GETJSAMPLE(colormap[1][i]) >> cshift, dinfo->pub.output_file);
	  fputc(GETJSAMPLE(colormap[2][i]) >> cshift, dinfo->pub.output_file);
	} else {
  ...
  fputc(',', dinfo->pub.output_file); /* separator */
  put_word(dinfo, 0);		/* left/top offset */
  put_word(dinfo, 0);
  put_word(dinfo, (unsigned int) dinfo->cinfo->output_width); /* image size */
  put_word(dinfo, (unsigned int) dinfo->cinfo->output_height);
  /* flag byte: not interlaced, no local color map */
  fputc(0x00, dinfo->pub.output_file);
  /* Write Initial Code Size byte */
  fputc(InitCodeSize, dinfo->pub.output_file);
...
finish_output_gif (j_decompress_ptr cinfo, djpeg_dest_ptr dinfo)
{
  ...
  fputc(0, dest->pub.output_file);
  /* Write the GIF terminator mark */
  fputc(';', dest->pub.output_file);
