...
void *mm_malloc(size_t size)
{
    ...
    if (p == free_list) {
      ...
      prevPtr = PREV_BLKP(ADD_PTR(mem_heap_hi(), 1));
      if (prevPtr < mem_heap_hi() && !GET_ALLOC(HDRP(prevPtr))) {
        // calculate new size to sbrk
        allocSize = newsize - GET_SIZE(HDRP(prevPtr));
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if (NEXT_BLKP(ptr) == ADD_PTR(mem_heap_hi(), 1)) {
      allocSize = realloc_size(size, OVERHEAD) - blkSize;
      mem_sbrk(allocSize);
...
*/
    newptr = mm_malloc((unsigned)size);
    if (newptr == NULL)
      return NULL;

    cpysize = MIN(blkSize - OVERHEAD, (unsigned)size);
